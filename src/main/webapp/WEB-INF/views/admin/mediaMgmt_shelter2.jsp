...
<body>
	...
	<div class="content-box-large">
		<div class="panel-heading">
			<div class="panel-title" style="font-size: 30px;">중앙차로 쉘터</div>
		</div>
		<div class="submenubtn"
			...
			<button class="btn btn-primary"
				...
				쉘터</button>
			<button class="btn btn-primary"
				style="background-color: white; color: black; pointer-events: none;">중앙차로
				쉘터</button>
			<button class="btn btn-primary"
		...
		<div style="float: left; padding-left: 15px; padding-top: 15px;">
			<select id="cntPerPage" name="sel" onchange="shelter2_selChange()">
				<option value="10"
		...
		<div style="width: 300px; float: right; padding-left: 90px;">
			...
			<div>
				<button style="float: left; height: 20px; margin-top: 15px;"
					name="btnSearch" id="shelter2_btnSearch">검색</button>
			</div>
		</div>
		<div class="panel-body">
			...
			<div style="display: block; text-align: center;">
				<c:if test="${paging.startPage != 1 }">
					<a class="pg_prev"
						href="mediaMgmt_shelter2?nowPage=${paging.nowPage - 1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">←
						Previous</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
					...
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a class="pg_page"
								href="mediaMgmt_shelter2?nowPage=${p }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a class="pg_next"
						href="mediaMgmt_shelter2?nowPage=${paging.nowPage+1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">Next
						→ </a>
				</c:if>
			</div>
		</div>
