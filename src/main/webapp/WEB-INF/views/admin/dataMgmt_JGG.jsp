...
<body>
	<div class="content-box-large">
		<div class="panel-heading">
			<div class="panel-title" style="font-size: 30px;">데이터 관리 -
				생활인구(집계구)</div>
		</div>
		...
		<div class="submenubtn"
			...
			<button class="btn btn-primary"
				onclick="javascript:location.href='dataMgmt'">지하철승하차인원</button>
			<button class="btn btn-primary"
				onclick="javascript:location.href='dataMgmt_HJD'">생활인구(행정동)</button>
			<button class="btn btn-primary"
				style="background-color: white; color: black; pointer-events: none;">생활인구(집계구)</button>
		</div>
						...
						<tr class="odd gradeX">
							...
							<th style="width: 100px;">
								<button class="btn btn-info"
									style="width: 80px; height: 20px; padding: 0;"
									onclick="apiInfoUpdate();">
			...
			<div class="logList">
				<c:forEach var="log" items="${ScheduledLog}" varStatus="status">
					<div class="loginfo errorCk${status.index}">
						...
						<c:choose>
							<c:when test="${log.useDt eq ''}">
								<div class="logMsg">사용일자 : ERROR</div>
							</c:when>
							<c:otherwise>
								<div class="logMsg">
									사용일자 :
									<fmt:parseDate value='${log.useDt}' var='useDt'
										pattern='yyyymmdd' />
									<fmt:formatDate value="${useDt}" pattern="yyyy-mm-dd" />
								</div>
							</c:otherwise>
						</c:choose>
