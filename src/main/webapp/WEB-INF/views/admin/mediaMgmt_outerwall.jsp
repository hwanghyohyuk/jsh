...
<body>
	...
	<div class="content-box-large">
		...
		<div class="submenubtn"
			...
			<button class="btn btn-primary"
				style="background-color: white; color: black; pointer-events: none;">빌딩외벽</button>
			<a href="#" data-toggle="modal" data-target="#OuterwallExcelupload"
		...
		<div style="float: left; padding-left: 15px; padding-top: 15px;">
			<select id="cntPerPage" name="sel" onchange="outerwall_selChange()">
				<option value="10"
		...
		<div style="width: 300px; float: right; padding-left: 90px;">
			...
			<div>
				<button style="float: left; height: 20px; margin-top: 15px;"
					name="btnSearch" id="outerwall_btnSearch">검색</button>
			</div>
		</div>
		<div class="panel-body">
			<table cellpadding="0" cellspacing="0" border="0"
				...
				<tbody>
					<c:forEach var="OuterwallList" items="${viewAll}"
						varStatus="status">
						<tr class="odd gradeX">
							<td id=bdNm${status.index}>${OuterwallList.bdNm}</td>
							<td id=acptLawDvsnNm${status.index}>${OuterwallList.acptLawDvsnNm}</td>
							<td id=roadAddress${status.index}>${OuterwallList.roadAddress}</td>
							<td id=adType${status.index}>${OuterwallList.adType}</td>
							<td id=material${status.index}>${OuterwallList.material}</td>
							<td id=lighting${status.index}>${OuterwallList.lighting}</td>
							<td id=installQt${status.index}>${OuterwallList.installQt}</td>
							<td id=term${status.index}>${OuterwallList.term}</td>
							<td id=adPrice${status.index}>${OuterwallList.adPrice}</td>
							<td id=adMakePrice${status.index}>${OuterwallList.adMakePrice}</td>
							<td id=size${status.index}>${OuterwallList.size}</td>
							<td id=mediaCompany${status.index}>${OuterwallList.mediaCompany}<input
								type="hidden" id=gid${status.index} name="gid"
								value="${OuterwallList.gid}"></td>
							<th style="text-align: center;">
								<button class="btn btn-info"
									style="width: 55px; height: 20px; padding: 0;"
									onclick="outerwall_dbupdate(<c:out value="${status.index}"/>);">
									<i class="glyphicon glyphicon-refresh"></i>변경
								</button>
								<button class="btn btn-danger"
									style="width: 55px; height: 20px; padding: 0;"
									onclick="outerwall_dbdelete(<c:out value="${status.index}"/>);">
			...
			<div style="display: block; text-align: center;">
				<c:if test="${paging.startPage != 1 }">
					<a class="pg_prev"
						href="mediaMgmt_outerwall?nowPage=${paging.nowPage - 1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">←
						Previous</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
					...
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a class="pg_page"
								href="mediaMgmt_outerwall?nowPage=${p }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a class="pg_next"
						href="mediaMgmt_outerwall?nowPage=${paging.nowPage+1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">Next
						→ </a>
				</c:if>
			</div>
		</div>
