...
<body>
	...
	<div class="content-box-large">
		...
		<div class="submenubtn"
			...
			<button onclick="setIntegrationModal('edisplay')" data-toggle="modal"
		...
		<div class="panel-body">
				...
				<tbody>
					<c:forEach var="EdisplayList" items="${viewAll}" varStatus="status">
						<tr class="odd gradeX">
							...
							<td id=mediaCompany${status.index}>${EdisplayList.mediaCompany }</td>
							<td style="text-align: center; width: 300px;">
								<button onclick="modalValput(${EdisplayList.gid})"
			...
			<div style="display: block; text-align: center;">
				<c:if test="${paging.startPage != 1 }">
					<a class="pg_prev"
						href="popupImg_edisplay?nowPage=${paging.nowPage - 1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">←
						Previous</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
					...
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a class="pg_page"
								href="popupImg_edisplay?nowPage=${p }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a class="pg_next"
						href="popupImg_edisplay?nowPage=${paging.nowPage+1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">Next
						→ </a>
				</c:if>
			</div>
		</div>
