...
<body>
	...
	<div class="content-box-large">
		...
		<div class="submenubtn"
			...
			<button onclick="setIntegrationModal('townboard')"
		...
		<div class="panel-body">
					...
					<c:forEach var="TownboardList" items="${viewAll}"
						...
						<tr class="odd gradeX">
							...
							<td style="text-align: center; width: 300px;">
								<button onclick="modalValput(${TownboardList.gid})"
			...
			<div style="display: block; text-align: center;">
				<c:if test="${paging.startPage != 1 }">
					<a class="pg_prev"
						href="popupImg_townboard?nowPage=${paging.nowPage - 1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">←
						Previous</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
					...
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a class="pg_page"
								href="popupImg_townboard?nowPage=${p }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a class="pg_next"
						href="popupImg_townboard?nowPage=${paging.nowPage+1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">Next
						→ </a>
				</c:if>
			</div>
		</div>
