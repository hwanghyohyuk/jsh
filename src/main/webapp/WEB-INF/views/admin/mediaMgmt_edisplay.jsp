...
<body>
	...
	<div class="content-box-large">
		<div class="panel-heading">
			<div class="panel-title" style="font-size: 30px;">디지털빌보드</div>
		</div>
		<div class="submenubtn"
			...
			<button class="btn btn-primary"
				style="background-color: white; color: black; pointer-events: none;">디지털빌보드</button>
			<button class="btn btn-primary"
			...
			<a href="#" data-toggle="modal" data-target="#EdisplayExcelupload"
		...
		<div style="float: left; padding-left: 15px; padding-top: 15px;">
			<select id="cntPerPage" name="sel" onchange="edisplay_selChange()">
				<option value="10"
		...
		<div style="width: 300px; float: right; padding-left: 90px;">
			...
			<div>
				<button style="float: left; height: 20px; margin-top: 15px;"
					name="btnSearch" id="edisplay_btnSearch">검색</button>
			</div>
		</div>
		...
		<div class="panel-body">
			<table cellpadding="0" cellspacing="0" border="0"
				...
				<tbody>
					<c:forEach var="EdisplayList" items="${viewAll}" varStatus="status">
						<tr class="odd gradeX">
							<td id=name${status.index}>${EdisplayList.name}</td>
							<td id=address${status.index}>${EdisplayList.address}</td>
							<td id=size${status.index}>${EdisplayList.size}</td>
							<td id=spec${status.index}>${EdisplayList.spec}</td>
							<td id=adPriceMonth${status.index}>${EdisplayList.adPriceMonth}</td>
							<td id=operationTime${status.index}>${EdisplayList.operationTime}</td>
							<td id=mediaCompany${status.index}>${EdisplayList.mediaCompany }<input
								type="hidden" id=gid${status.index} name="gid"
								value="${EdisplayList.gid}"></td>
							<th style="text-align: center;">
								<button class="btn btn-info"
									style="width: 55px; height: 20px; padding: 0;"
									onclick="edisplay_dbupdate(<c:out value="${status.index}"/>);">
									<i class="glyphicon glyphicon-refresh"></i>변경
								</button>
								<button class="btn btn-danger"
									style="width: 55px; height: 20px; padding: 0;"
									onclick="edisplay_dbdelete(<c:out value="${status.index}"/>);">
			...
			<div style="display: block; text-align: center;">
				<c:if test="${paging.startPage != 1 }">
					<a class="pg_prev"
						href="mediaMgmt_edisplay?nowPage=${paging.nowPage - 1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">←
						Previous</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
					...
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a class="pg_page"
								href="mediaMgmt_edisplay?nowPage=${p }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a class="pg_next"
						href="mediaMgmt_edisplay?nowPage=${paging.nowPage+1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">Next
						→ </a>
				</c:if>
			</div>
		</div>
