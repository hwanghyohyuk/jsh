...
<body>
	...
	<div class="content-box-large">
		...
		<div class="submenubtn"
			...
			<a href="#" data-toggle="modal" data-target="#GdiaExcelupload"
		...
		<div class="panel-body">
				...
				<tbody>
					<c:forEach var="GdiaList" items="${viewAll}" varStatus="status">
						<tr class="odd gradeX">
							...
							<td id=quantity${status.index}>${GdiaList.quantity}</td>
							<td id=unitPrice${status.index}>${GdiaList.unitPrice}</td>
							<td id=adPriceMonth${status.index}>${GdiaList.adPriceMonth }<input
								type="hidden" id=gid${status.index} name="gid"
								value="${GdiaList.gid}"></td>
							<th style="text-align: center;">
								<button class="btn btn-info"
									style="width: 55px; height: 20px; padding: 0;"
									onclick="gdia_dbupdate(<c:out value="${status.index}"/>);">
									<i class="glyphicon glyphicon-refresh"></i>변경
								</button>
								<button class="btn btn-danger"
									style="width: 55px; height: 20px; padding: 0;"
									onclick="gdia_dbdelete(<c:out value="${status.index}"/>);">
			...
			<div style="display: block; text-align: center;">
				<c:if test="${paging.startPage != 1 }">
					<a class="pg_prev"
						href="mediaMgmt_gdia?nowPage=${paging.nowPage - 1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">←
						Previous</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
					...
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a class="pg_page"
								href="mediaMgmt_gdia?nowPage=${p }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a class="pg_next"
						href="mediaMgmt_gdia?nowPage=${paging.nowPage+1 }&cntPerPage=${paging.cntPerPage}&keyword=${paging.keyword}">Next
						→ </a>
				</c:if>
			</div>
		</div>
