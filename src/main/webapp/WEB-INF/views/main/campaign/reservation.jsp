...
<link rel="stylesheet" href="/resources/main/css/campaign/reservation.css">
</head>
<body>
	
	<c:set var="mediaList" value="${campaign.medialist }"></c:set>
	<c:set var="facilList" value="${campaign.facillist }"></c:set>
	<c:set var="mediaMixInfo" value="${campaignMap.mediaMixInfo }"></c:set>
	<input type="hidden" id="minag" name="minag" value="${params.minag}">
	<input type="hidden" id="maxag" name="maxag" value="${params.maxag}">
	...
	<div id="container">
		...
		<div class="create-window shadow">
			<div class="create-window-part absolute-lt bd-bt-1" id="info-part">
				<div class="info">
					<div class="info-header">
						<b class="info-title-l w-75">예약하기</b> 
					</div>
					<div class="info-body">
						<div>
							<b class="info-title">캠페인명</b><span>${ params.cpn }</span>
						</div>
						<div>
							<b class="info-title">광고주</b><span>${ params.adv }</span>
						</div>
						<div>
							<b class="info-title">일정</b><span>${ params.trm }</span>
						</div>
						<div>
							<b class="info-title">타게팅</b><span><c:choose><c:when test="${ params.cpt eq 'every' }">남성+여성</c:when><c:when test="${ params.cpt eq 'male' }">남성</c:when><c:when test="${ params.cpt eq 'female' }">여성</c:when></c:choose></span><span> ${ params.minag }${ params.maxag }</span>
						</div>
					</div>
				</div>
			</div>
			<div class="create-inner-window">
				<div class="create-window-part">
						...
						<div class="info-body">
							<div class="item-list">
								...
								<div class="item" id="mix-idx-${i.index }" onclick="itemAClick(${status.index})">
									<span class="col-6"><span class="unicon"><i class="${i.unicon }"></i></span><span class="m-title">
										...
										<c:if test="${i.dvsnA eq '2' and i.dvsnB eq '2' }">GDIA</c:if>
									...
									<span class="col-4 text-right adPriceMonth">미디어 로딩 중</span>
								</div>
								</c:if>
								</c:forEach>
							</div>
						...
						<div class="info-body">
							<div class="item-list">
								<div class="item-l my-2">
									...
									<span class="col-7 text-right"><fmt:formatNumber type="number" value="${mediaMixInfo.targetOts}"></fmt:formatNumber></span>
								</div>
								<div class="item-l my-2">
									<span class="col-5"><b>예상 총 OTS</b></span>
									<span class="col-7 text-right"><fmt:formatNumber type="number" value="${mediaMixInfo.totalOts}"></fmt:formatNumber></span>
								</div>
							</div>
						</div>
					</div>
					<div class="info bd-bt-1">
						<div class="info-header flex my-2">
							...
							<fmt:parseNumber var="adPriceMonth" type="number"  integerOnly="true" value="${mediaMixInfo.adPriceMonth}" />
							<fmt:parseNumber var="adMakePrice" type="number"  integerOnly="true" value="${mediaMixInfo.adMakePrice}" />
							<b class="info-title fc-blue px-0 col-7 text-right"><fmt:formatNumber type="number" value="${adPriceMonth + adMakePrice}"></fmt:formatNumber>원</b> 
						</div>
						<div class="info-body">
							<div class="item-list">
								<div class="item-l my-2">
									<span class="col-5"><b>광고료</b></span>
									<span class="col-7 text-right"><fmt:formatNumber type="number" value="${mediaMixInfo.adPriceMonth}"></fmt:formatNumber>원</span>
								</div>
								<div class="item-l my-2">
									<span class="col-5"><b>제작비</b></span>
									<span class="col-7 text-right"><fmt:formatNumber type="number" value="${mediaMixInfo.adMakePrice}"></fmt:formatNumber>원</span>
								</div>
							</div>
						</div>
					</div>
			 ...
			 <form name="imgForm" id="imgForm" action="/campaign/main/image/upload?${_csrf.parameterName}=${_csrf.token}" method="post" enctype="multipart/form-data">
			 	...
			 	<input type="hidden" id="state" name="state" value="reservation">
       			<input type="hidden" name="imgSrc" id="imgSrc" >
   			 </form>
   			 
			<div class="d-flex absolute-lb2" id="decide-part">
				<input type="button" class="btn btn-save btn-no-radius flex-fill" id="cancel" value="취소" /> 
				<input type="button" class="btn btn-primary btn-no-radius flex-fill" id="confirm" value="확인" />
			</div>
		</div>
	</div>
	<div class="create-sub-window shadow">
			<div class="create-window-part absolute-lt" id="sub-part">
				<div class="info pt-3 px-4">
					...
					<div class="info-body row totalAdInfo">
						<div class="col-7 px-0">
							...
							<b class="info-title w-40 mr-3 mb-1">제작비</b><span class="adMakePrice">-</span><br>
							<b class="info-title w-40 mr-3 mb-1">예상 타겟 OTS</b><span class="targetOts">-</span><br>
							<b class="info-title w-40 mr-3 mb-1">예상 총 OTS</b><span class="totalOts">-</span>
						</div>
						<div class="col-5 totalAdRating">
						<!-- 등급 -->
						</div>
					</div>
				</div>
	...
	<script src="/resources/main/js/campaign/reservation.js"></script>
